...
# Live switch:
# - MR_PAPER=true  -> write paper=true orders
# - MR_PAPER=false -> write paper=false orders (live pipeline can pick them up)
PAPER = env_bool("MR_PAPER", True)
...
def main():
    ...
    print(
        ...
        f"mode={mode} poll={POLL_SECS}s batch={BATCH}"
    )
        ...
        while True:
            with conn.cursor() as cur:
                ...
                for it in intents:
                    ...
                    # IMPORTANT:
                    # Your DB has a partial unique index:
                    #   uq_strategy_orders_intent_id UNIQUE(intent_id) WHERE intent_id IS NOT NULL
                    # So you must reference the index/constraint name, not ON CONFLICT (intent_id) WHERE ...
                    cur.execute(
                        """
                    ...
                    if row is None:
                        # conflict: order already exists, fetch it
                        cur.execute(
                    ...
                    print(
                        ...
                        f"market={market_id} outcome={outcome} px={limit_px} qty={qty} mode={mode}"
                    )
